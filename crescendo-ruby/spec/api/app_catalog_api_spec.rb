=begin
#Crescendo API

#The Crescendo API powers workflow automation and transactional email at scale. Authenticate with a Bearer API key (prefix: `re_...`) obtained from the dashboard.

The version of the OpenAPI document: v1

Generated by: https://openapi-generator.tech
Generator version: 7.23.0

=end

require 'spec_helper'
require 'json'

# Unit tests for Crescendo::AppCatalogApi
# Automatically generated by openapi-generator (https://openapi-generator.tech)
# Please update as you see appropriate
describe 'AppCatalogApi' do
  before do
    # run before each test
    @api_instance = Crescendo::AppCatalogApi.new
  end

  after do
    # run after each test
  end

  describe 'test an instance of AppCatalogApi' do
    it 'should create an instance of AppCatalogApi' do
      expect(@api_instance).to be_instance_of(Crescendo::AppCatalogApi)
    end
  end

  # unit tests for get_app
  # Get app details
  # Gets triggers, actions, and connection schemas for a specific app. Requires app:read scope.
  # @param app_key 
  # @param [Hash] opts the optional parameters
  # @return [AppDetailResponse]
  describe 'get_app test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

  # unit tests for list_apps
  # List apps
  # Lists all supported apps in the catalog. Requires app:read scope.
  # @param [Hash] opts the optional parameters
  # @return [Array<AppSummaryResponse>]
  describe 'list_apps test' do
    it 'should work' do
      # assertion here. ref: https://rspec.info/features/3-12/rspec-expectations/built-in-matchers/
    end
  end

end
