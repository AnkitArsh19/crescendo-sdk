...
module Crescendo
  class CreateTemplateRequest < ApiModelBase
    attr_accessor :name
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crescendo::CreateTemplateRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crescendo::CreateTemplateRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @subject.to_s.length < 0
        invalid_properties.push('invalid value for "subject", the character length must be greater than or equal to 0.')
      end
    ...
    def valid?
      ...
      return false if @subject.to_s.length > 1000
      return false if @subject.to_s.length < 0
    ...
    def subject=(subject)
      ...
      if subject.to_s.length < 0
        fail ArgumentError, 'invalid value for "subject", the character length must be greater than or equal to 0.'
      end
    ...
    def hash
      [name, subject, html_body, text_body, variables].hash
    end
