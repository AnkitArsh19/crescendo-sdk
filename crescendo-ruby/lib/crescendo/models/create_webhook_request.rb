  ...
  class CreateWebhookRequest < ApiModelBase
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crescendo::CreateWebhookRequest` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crescendo::CreateWebhookRequest`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @subscribed_events.length < 1
        invalid_properties.push('invalid value for "subscribed_events", number of items must be greater than or equal to 1.')
      end
    ...
    def valid?
      ...
      return false if @subscribed_events.length < 1
      true
    end
    ...
    # Custom attribute writer method with validation
    # @param [Object] subscribed_events Value to be assigned
    def subscribed_events=(subscribed_events)
      if subscribed_events.nil?
        fail ArgumentError, 'subscribed_events cannot be nil'
      end

      if subscribed_events.length < 1
        fail ArgumentError, 'invalid value for "subscribed_events", number of items must be greater than or equal to 1.'
      end
