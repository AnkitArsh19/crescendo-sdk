...
module Crescendo
  class MetricsSummary < ApiModelBase
    attr_accessor :total
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Crescendo::MetricsSummary` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Crescendo::MetricsSummary`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'total_opens')
        self.total_opens = attributes[:'total_opens']
      end

      if attributes.key?(:'total_clicks')
    ...
    def hash
      [total, pending, sent, delivered, failed, bounced, suppressed, total_opens, total_clicks].hash
    end
